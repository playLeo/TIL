...
Docker는 같은 커널에 기초하기만 하면 어떤 운영 체제라도 실행할 수 있다.(CentOs, Debian, Fedora...)

Linux가 설치된 Docker 호스트에서는 Windows기반 컨테이너를 실행할 수 없다. 따라서 Windows 서버에 Docker를 설치해야한다.

이러한 문제점을 해결하기 위해, WLS를 사용해서 Linux 시스템을 실행하는 가상머신을 만든다.
